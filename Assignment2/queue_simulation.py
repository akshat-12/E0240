...
def simulate_queue(injection_rate, service_time_sampler, simulation_cycles, rng):
    ...
    for _ in range(simulation_cycles):
        ...
        if remaining_service_time == 0 and queue_length > 0:
            queue_length -= 1
            remaining_service_time = service_time_sampler(rng)
...
def deterministic_service_time(_rng):
    return 2
...
def plot_results(results):
    ...
    figure.tight_layout()
    figure.savefig("queue_simulation.png", dpi=300, bbox_inches="tight")
    plt.show()
